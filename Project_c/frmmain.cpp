    ...
    ui(new Ui::frmMain)
{
    ...
    /* 连接已建立 -> 开始发数据 */
    connect(sendf, SIGNAL(connected()),this, SLOT(start_transfer()));
    /* 数据已发出 -> 继续发 */
    connect(sendf, SIGNAL(bytesWritten(qint64)),this, SLOT(continue_transfer(qint64)));
    /* socket出错 -> 错误处理 */
    connect(sendf, SIGNAL(error(QAbstractSocket::SocketError)),this, SLOT(show_error(QAbstractSocket::SocketError)));
}
...
void frmMain::on_pushButton_clicked()
{
    ...
    string key = qkey.toStdString();
    //加密
    string str_out= RC4(key, str, temp);
    //密文
    QString sresult = QString::fromStdString(str_out);
...
{
    memset(md5_char,0,sizeof(char)*100);
    QString str = this -> ui -> lineEdit_5 -> text();
    string st = str.toStdString();
    cacu_md5(st, md5_char);
    string st2 = md5_char;
    QString str2 = QString::fromStdString(st2);
    this -> ui -> lineEdit_6 -> setText(str2);
...
/*void frmMain::show_error(QAbstractSocket::SocketError)
{
    ...
    /*--- 点击选择按钮 ---*/
    void frmMain::on_selectBtn_clicked()
    {
        /* 开文件选择窗选文件，返回带路径文件名 */
    ...
    /*--- 点击发送按钮 ---*/
    void frmMain::on_sendBtn_clicked()
    {
        /* 发送连接请求 */
        sendf->connectToHost(QHostAddress::LocalHost, PORT);
        sentBytes = 0;
    ...
    void frmMain::start_transfer()
    {
        ...
        /* 发送首部，计算剩余大小 */
        restBytes = fileBytes - sendf->write(buf);
    }

    /*--- 继续传输 ---*/
    void frmMain::continue_transfer(qint64 sentSize)
    {
        sentBytes += sentSize;
